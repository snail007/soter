...
class TestDataCheck extends UnitTestCase {
    ...
    public function testreg() {
        $param = array('username' => 'h123456');
        $rule = array(
            'username' => array(
                'required' => '用户名称必须填写',
                'min_len[5]' => '用户名称太短',
                'max_len[45]' => '用户名称最多45字符',
                'reg[/^[a-zA-Z0-9]{5,10}$/]#' => '用户名为5，20位英文字母和数字的组合',
            ),
        );
        $isOkay = Sr::checkData($param, $rule, $retData, $errorMsg);
        if (!$isOkay) {
            return Sr::json(-1, $errorMsg);
        }
